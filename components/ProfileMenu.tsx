...
export const ProfileMenu: React.FC = () => {
  const { user, signOut, hardSignOut, signInWithGoogle, loading, error, clearError } = useAuth();
  ...
  const renderAuthError = () =>
    error ? (
      <TouchableOpacity onPress={clearError} style={{ margin: 10, alignSelf: 'center' }}>
        <Text style={{ color: 'red', fontWeight: 'bold' }}>{error} (tap to dismiss)</Text>
      </TouchableOpacity>
    ) : null;

  // Handle sign out (now uses hardSignOut)
  const handleMenuSignOut = async () => {
    setMenuVisible(false);
    console.log('[MENU] Sign Out pressed');
    await hardSignOut(
      () => { if (typeof window !== 'undefined') window.location.reload(); },
      (err) => { if (typeof window !== 'undefined') window.alert('Failed to sign out. Please try again.'); else Alert.alert('Error', 'Failed to sign out. Please try again.'); }
    );
  };
  ...
  const handleSwitchAccount = async () => {
    setMenuVisible(false);
    console.log('[MENU] Switch Account pressed');
    await hardSignOut(
      async () => {
      ...
      (err) => { if (typeof window !== 'undefined') window.alert('Failed to switch account. Please try again.'); else Alert.alert('Error', 'Failed to switch account. Please try again.'); }
    );
  };
  ...
  return (
    <>
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          if (!loading) setMenuVisible(true);
        }}
        activeOpacity={0.6}
      ...
      >
        <TouchableWithoutFeedback onPress={() => {
          console.log('[DEBUG] Modal overlay pressed - closing menu');
          setMenuVisible(false);
        }}>
        ...
        <View
          style={[styles.menuContainer, { backgroundColor: themeStyles.menuBackgroundColor }]}
          onStartShouldSetResponder={() => {
            console.log('[DEBUG] Menu container onStartShouldSetResponder');
            return false;
          }}
          onTouchStart={() => {
            console.log('[DEBUG] Menu container onTouchStart');
          }}
        >
          ...
          <View style={styles.menuItems}>
            {menuItems.map((item, index) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  console.log(`[DEBUG] Menu item pressed: ${item.title}`);
                  item.onPress();
                }}
                activeOpacity={0.5}
              >
      ...
      {/* Debug Sign Out Button - For development only */}
      {typeof __DEV__ !== 'undefined' && __DEV__ && (
        <TouchableOpacity
          style={{ margin: 20, padding: 10, backgroundColor: 'red' }}
          onPress={async () => {
            console.log('[MENU] DEBUG SIGN OUT pressed');
            try {
              await signOut();
        ...
        >
          <Text style={{ color: 'white', fontWeight: 'bold' }}>DEBUG SIGN OUT</Text>
        </TouchableOpacity>
      )}
    </>
  );
